...
public partial class MainWindowViewModel : ViewModelBase
{
    ...
    [ObservableProperty]
    private bool _isDesignerVisible = false;
    ...
    private void OpenDesigner()
    {
        ...
        try
        {
            Log.Information("OpenDesigner: Opening designer for template {TemplateId} - {TemplateName}",
                SelectedTemplate.Id, SelectedTemplate.Name);

            Log.Debug("OpenDesigner: Getting ReportDesignerViewModel from DI container");
            DesignerViewModel = _serviceProvider.GetRequiredService<ReportDesignerViewModel>();

            Log.Debug("OpenDesigner: Loading template async");
            _ = DesignerViewModel.LoadTemplateAsync(SelectedTemplate.Id);

            Log.Debug("OpenDesigner: Switching to designer view");
            IsDesignerVisible = true;

            StatusMessage = $"Editing '{SelectedTemplate.Name}'";
    ...
    {
        Log.Information("CloseDesigner called");
        IsDesignerVisible = false;
        DesignerViewModel = null;
        StatusMessage = "Ready";
        Log.Information("CloseDesigner: Returned to template list");
    }
}
